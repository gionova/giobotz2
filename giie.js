...
Gionova *X* Echa☕︎`
sendButLocation(from, `*GionovaBots | JavaScript Source Code* \n_( If we never try, we will never know )_`, `${menu}`, {jpegThumbnail:ganz}, [{buttonId:`.allmenu`,buttonText:{displayText:'All Festure︎'},type:1},{buttonId:`.command`,buttonText:{displayText:'List Menu'},type:1},{buttonId:`.owner`,buttonText:{displayText:'Developer | Creator'},type:1}], {contextInfo: { mentionedJid: [ptod]}})
...
break
case 'giie':
buttonss = [{buttonId: `${prefix}igstalk giiexsala`, buttonText: {displayText: 'INSTAGRAM🩰'}, type: 1},{buttonId: `${prefix}creator`, buttonText: {displayText: 'WHATSAPP🧸'}, type: 1}]
const bum = {
...
break
        case 'indo':
				...
				crew = body.slice(11)
late = crew.split("|")[0];
				anu = await fetchJson(`https://bx-hunter.herokuapp.com/api/translate?lang=d&text=${late}&apikey=${HunterApi}`, {method: 'get'})
				teks = anu.text
reply(teks)
break
		case 'artimimpi':
...
giie.sendMessage(from, hah, audio, {mimetype: 'audio/mp4', duration: 359996400, ptt:true, quoted: mek, contextInfo: { forwardingScore: 508, isForwarded: true}})
fs.unlinkSync(ran)
})
break
case 'gemuk':
...
case 'matrix':
				...
				reply(mess.wait)
				anu1 = await getBuffer(`https://bx-hunter.herokuapp.com/api/flamingtext/text3d?text=${arg}&apikey=${HunterApi}`)
                    giie.sendMessage(from, anu1, image, {quoted: mek, caption : `${prefix}sticker`})
				break
				case 'simg':
				var imgbb = require('imgbb-uploader')
				if (isMedia || isQuotedImage) {
					const shimgoh = isQuotedImage ? JSON.parse(JSON.stringify(mek).replace('quotedM','m')).message.extendedTextMessage.contextInfo : mek 
					const mediaData = await  giie.downloadAndSaveMediaMessage(shimgoh)
					await reply(mess.wait)
					anu = await imgbb("0ffc503f79f9b051b82e643eb3e3a746", mediaData)
					teks = `${anu.display_url}`
					const sbrowser = await puppeteer.launch(options)
					const spage = await sbrowser.newPage()
					await spage.goto('https://www.google.com/searchbyimage?image_url=${teks}')
...
break
 ...
 anu =`*WELCOME*\n_Please Click Button Dibawah !_\n\n*ON*: untuk mengaktifkan\n*OFF*: untuk menonaktifkan`
punten = [{buttonId: 'welcome on', buttonText: {displayText: 'ON ☺︎︎'}, type: 1},{buttonId: 'welcome off', buttonText: {displayText: 'OFF ☹︎'}, type: 1}]
const btnasu = {
    contentText: `${anu}`,
    footerText: '*_Developer by: AyogiAk|JavaScript_*',
    buttons: punten,
...
await giie.sendMessage(from, btnasu, MessageType.buttonsMessage, {quoted: fgi})
 ...
 anu =`*ANTI DELETE*\n_Please Click Button Dibawah !_\n\n*ON*: untuk mengaktifkan\n*OFF*: untuk menonaktifkan`
punten = [{buttonId: 'antidelete on', buttonText: {displayText: 'ON ☺︎︎'}, type: 1},{buttonId: 'antidelete off', buttonText: {displayText: 'OFF ☹︎'}, type: 1}]
const btnasu = {
    contentText: `${anu}`,
    footerText: '*_Developer by: AyogiAk|JavaScript_*',
    buttons: punten,
                    ...
                    case 'play':
                            ...
                            try {
                                ...
                                .then((res) => {
                                    ...
                                    sendFileFromUrl(dl_link, document, {mimetype: 'audio/mp3', ptt:true, duration: 359996400, filename: `${title}.mp3`, quoted: mek, contextInfo: { forwardingScore: 508, isForwarded: true, externalAdReply:{title:title,body:"",mediaType:"2",thumbnail:getBuffer(thumb),mediaUrl:`${body.slice(7)}`}}}).catch(() => reply(mess.error.api))
                                    })
                                })
                            } catch (err) {
                                reply(mess.error.api)
